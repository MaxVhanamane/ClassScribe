...
export default async function handler(req, res) {
    ...
    if (req.method === "POST") {
        try {
            let deletedStudentInfo = await Student.findOneAndDelete(req.body)

            await AttendanceRecord.deleteMany({ name: req.body._id, DOB: req.body.DOB })
            
            if (deletedStudentInfo) {
